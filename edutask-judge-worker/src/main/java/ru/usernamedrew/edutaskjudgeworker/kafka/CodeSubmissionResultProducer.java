...
public class CodeSubmissionResultProducer {
    private final KafkaTemplate<Object, Object> kafkaTemplate;
    private final TaskGenerationKafkaProperties properties;

    public CompletableFuture<SendResult<Object, Object>> send(CodeSubmissionResultEvent event) {
        Objects.requireNonNull(event, "event must not be null");
        Objects.requireNonNull(event.submissionId(), "event.submissionId must not be null");

        String topic = properties.getTopics().getCodeSubmissionResults();
        String key = event.submissionId().toString();
        return kafkaTemplate.send(topic, key, event)
            .whenComplete((result, exception) -> {
                if (exception != null) {
                    log.error("Failed to publish code submission result, submissionId={}", event.submissionId(), exception);
                    return;
                }
                log.debug(
                    "Published code submission result, submissionId={}, topic={}, partition={}, offset={}",
                    event.submissionId(),
