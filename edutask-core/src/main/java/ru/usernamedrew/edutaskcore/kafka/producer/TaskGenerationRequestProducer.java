...
public class TaskGenerationRequestProducer {
    ...
    public CompletableFuture<SendResult<Object, Object>> send(TaskGenerationRequestedEvent event) {
        ...
        String topic = properties.getTopics().getTaskGenerationRequests();
        String key = event.requestId().toString();
        return kafkaTemplate.send(topic, key, event)
            .whenComplete((result, exception) -> {
                if (exception != null) {
                    log.error("Failed to publish task generation request, requestId={}", event.requestId(), exception);
                    return;
                }
                log.debug(
                    "Published task generation request, requestId={}, topic={}, partition={}, offset={}",
                    event.requestId(),
